...
import { ProxyProperty } from './config.decorator';
...
export class ConfigService {
  ...
  static get(param: string | string[], value: any = undefined): any {
    const configValue = lodash.get(ConfigService.config, param);
  ...
  static reset(newConfig: Config) {
    ConfigService.config = newConfig;
  }

  reset(newConfig: Config) {
    ConfigService.config = newConfig;
  }
  ...
  set(param: string | string[], value: any = null): Config {
    return lodash.set(ConfigService.config, param, value);
  }
  ...
  has(param: string | string[]): boolean {
    return lodash.get(ConfigService.config, param) !== undefined;
  }
